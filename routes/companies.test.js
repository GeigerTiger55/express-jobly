...
describe("POST /companies", function () {
  ...
  test("not admin not allowed", async function () {
    const resp = await request(app)
      .post("/companies")
      .send(newCompany)
  ...
  test("no user not allowed", async function () {
    const resp = await request(app)
      .post("/companies")
      .send(newCompany);
    expect(resp.statusCode).toEqual(401);
...
describe("GET /companies", function () {
  ...
  test("works with filters", async function () {
    const resp = await request(app).get(
      "/companies?minEmployees=1&maxEmployees=2&nameLike=2"
    );
    expect(resp.body).toEqual({
      companies:
        [
  ...
  test("returns error if invalid filters given", async function () {
    ...
    expect(resp.statusCode).toEqual(400);
    expect(resp.body.error.message).toEqual("Invalid filters given.");
  });
...
describe("PATCH /companies/:handle", function () {
  ...
  test("not admin doesn't work for user", async function () {
    const resp = await request(app)
      .patch(`/companies/c1`)
      .send({
        name: "C1-new",
